...
module TextAnalyzer = 
    ...
    let analyzeText (text: string) =
        ...
        match cleanText with
        | "" | "Drag and drop a file here or paste your text..." -> 0, 0, 0, Seq.empty, Seq.empty, 0.0
        | _ ->
        ...
        let wordFrequency =
            words
            |> Seq.groupBy id //Groups words by their value
            |> Seq.map (fun (word, occurrences) -> word, Seq.length occurrences) //Creates a sequence of tuples where each tuple is a word and its count.
